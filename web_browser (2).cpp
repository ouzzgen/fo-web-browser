...
WebBrowser::WebBrowser(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(500, 500))
{
    ...
    helpMenu = new wxMenu;
    karalama = new wxMenu;
    helpTopicsItem = new wxMenuItem(helpMenu, id_help_topics_item, wxT("Help Topics"));
    developerItem = new wxMenuItem(helpMenu, id_developer_item, wxT("developer"));
    ...
    helpMenu->Append(helpTopicsItem);
...
*/
    ...
    Connect(id_ftp_file_download_item, wxEVT_COMMAND_MENU_SELECTED,
            wxMenuEventHandler(WebBrowser::FtpFileDownload));
                    ...
                    Connect(id_developer_item, wxEVT_COMMAND_MENU_SELECTED,
                        wxCommandEventHandler(WebBrowser::FuncAbout));
...
void WebBrowser::FuncAbout(wxCommandEvent &WXUNUSED(event))
{
    int wid = 22;
    int fid = 33;
    // wxWindow *abWin = new wxWindow();
    wxWindow *mainAboFrame = new wxWindow(this, wid, wxDefaultPosition, wxDefaultSize, 0, "he");
    // wxFrame *mainAboFrame = new wxFrame(this, wid, "hell", wxDefaultPosition, wxDefaultSize, 1, "k");
    // mainAboFrame->SetSize(300, 300);

     //wxFrame *wf = new wxFrame(mainAboFrame, fid, wxDefaultPosition, wxDefaultSize, 0, "he");
    // wxFrame *wf = new wxFrame(mainAboFrame, fid, "hell", wxDefaultPosition, wxDefaultSize, 1, "k");
    // wf->SetSize(300, 300);

    wxStaticBitmap *wsb = new wxStaticBitmap();
    wxBitmap bm = wxBitmap("minibenbro-1.jpg");
    wsb->SetBitmap(bm);
    // wsb->SetSize(30, 30);
    // wsb->SetSize(50, 50);

    // wxBoxSizer *wbsAbout = new wxBoxSizer(wxHORIZONTAL);
  wsb->SetParent(mainAboFrame);
    // wbsAbout->Add(wf, 1, wxEXPAND | wxALL, 20);
    // wxImage *wi = new wxImage("hello", wxBITMAP_TYPE_ANY, -1);
    // wxPanel *wpan = new wxPanel();
    // wi->LoadFile("benbro.jpg", 1, -1);

    // wpan->AddChild(wi);
    // wxObject *wo = bm->wxCreateObject();

    // mainAboFrame->SetSizer(wbsAbout, true);
    // mainAboFrame->AddChild(wf);

// mainAboFrame->SetSize(500, 500);
    mainAboFrame->Show(true);

    // wf->AddChild(wbsAbout);
    // wf->Show(true);
}
